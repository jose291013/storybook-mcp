...
- Current branch: `codex/provider-safe-structure-first`
...
## Candidate brick: provider-safe structure-first finishing
...
then edits that private foundation with the exact identity, outfit and approved
artistic-medium references under a short generic prompt. The finishing prompt
contains no customer names, photo fingerprints, reader prose or provider/QA
error sentences.

When the finished candidate still misses a strict domain, the only allowed
retry is another finishing edit of that candidate. Its corrections are compiled
from bounded issue codes into positive instructions for cast, wardrobe,
equipment, physical medium, boundary, action, objects, landmarks, identity or
style. The controller permits two QA attempts and at most three image calls in
total. Exhaustion returns the page directly to the durable private queue; the
old raw-text repair sweep is skipped. All 35 accepted pages remain immutable.

Verification: focused provider-safety and page-isolation tests pass 51/51 and
the complete repository regression passes 863/863 tests.
