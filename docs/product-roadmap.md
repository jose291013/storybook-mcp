...
5. Illustration QA rejects objective production defects such as corruption, blank output, repeated bands or incomplete rendering. A separate continuity comparison locks every interior illustration to the cover's rendering medium, so an obvious jump between photography, 3D, watercolor or flat cartoon is also a technical defect. If OpenAI rejects branded identity pixels for safety, Calitiki retries once with the already-derived non-branded identity canon while retaining the safe cover-style reference.
...
- The preview-ready email preference is durable. Completion sends an idempotent signed WooCommerce callback and records its sent timestamp; if that callback or SMTP fails, reopening the completed project retries the same notification rather than silently losing it. The photo-upload step advises using unbranded clothing, caps and toys without visible well-known characters to reduce avoidable image-safety refusals.
