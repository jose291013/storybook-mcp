...
test("preview generation quarantines one page, continues the book and performs a bounded repair sweep", async () => {
  ...
  assert.match(preview, /provider_safe_structure_first/);
  assert.match(preview, /providerSafetyMinimal: true/);
  assert.match(preview, /qualityReferenceImages/);
  assert.match(qualityGate, /providerSafetyStructureFirst \? Math\.min\(2, maximumAttempts\)/);
  assert.match(qualityGate, /phase: "foundation-generated"/);
  assert.match(qualityGate, /phase: "foundation-approved"/);
  assert.match(qualityGate, /phase: "foundation-rejected"/);
