...
export function buildFinalPrompt({
  ...
  const referenceContract = orderedReferences.length
    ? `\n\nREFERENCE IMAGE CONTRACT:\n${orderedReferences.map((item, index) => (
        `- Reference ${index + 1} [${item.kind === "repair_source" ? "TARGET IMAGE TO EDIT" : item.kind === "continuity" ? "PRIMARY APPROVED STYLE ANCHOR" : item.kind === "identity" ? "IDENTITY ONLY" : "SUPPORTING REFERENCE"}]: ${item.label || "visual continuity reference"}`
