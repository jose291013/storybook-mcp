...
export function compactImageSceneContract(contract = {}, aliases = [], { safetyFallback = false, pairedText = "" } = {}) {
  ...
  return {
    ...
    visual_composition: contract?.visual_composition ? {
      version: Number(contract.visual_composition?.version || 0),
      composition_id: safe(contract.visual_composition?.composition_id),
      framing: safe(contract.visual_composition?.framing),
      shot_scale: safe(contract.visual_composition?.shot_scale),
      viewpoint: safe(contract.visual_composition?.viewpoint),
    ...
    scene_density: contract?.scene_density ? {
      version: Number(contract.scene_density?.version || 0),
      age_band: safe(contract.scene_density?.age_band),
      density_mode: safe(contract.scene_density?.density_mode),
      high_salience_limit: Math.max(2, Math.min(3, Number(contract.scene_density?.high_salience_limit || 2))),
      decorative_detail_limit: Math.max(0, Number(contract.scene_density?.decorative_detail_limit || 0)),
      primary_focus: list(contract.scene_density?.primary_focus, 3).map(safe),
      supporting_cast: list(contract.scene_density?.supporting_cast, 10).map(safe),
      supporting_elements: list(contract.scene_density?.supporting_elements, 15).map(safe),
      background_states: list(contract.scene_density?.background_states, 20).map(safe),
      hierarchy_rule: safe(contract.scene_density?.hierarchy_rule),
      decoration_rule: safe(contract.scene_density?.decoration_rule),
    } : null,
