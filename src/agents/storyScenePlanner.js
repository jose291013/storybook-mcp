...
export function sceneContractImagePrompt({
  ...
  return [
    ...
    compact.visual_composition ? `LOCKED VISUAL COMPOSITION: ${compact.visual_composition.framing}; ${compact.visual_composition.shot_scale}; ${compact.visual_composition.viewpoint}; ${compact.visual_composition.subject_placement}; depth: ${compact.visual_composition.depth_plan}; rhythm: ${compact.visual_composition.visual_rhythm}; whole-book energy ${compact.visual_composition.energy_level}/5. ${compact.visual_composition.cast_readability}. ${compact.visual_composition.action_readability}. Do not replace this with the previous scene's pose, camera or layout.` : "",
    compact.scene_density ? `LOCKED SCENE DENSITY for age band ${compact.scene_density.age_band}: mode ${compact.scene_density.density_mode}; maximum ${compact.scene_density.high_salience_limit} high-salience entities; primary focus ${compact.scene_density.primary_focus.join(" + ")}; supporting cast ${compact.scene_density.supporting_cast.join(", ") || "none"}; supporting elements ${compact.scene_density.supporting_elements.join(", ") || "none"}; background states ${compact.scene_density.background_states.join(", ") || "none"}. ${compact.scene_density.hierarchy_rule} ${compact.scene_density.decoration_rule}` : "",
    ...
    compact.render_snapshot?.fixed_entities?.length ? `UNIQUE FIXED ENTITIES: ${compact.render_snapshot.fixed_entities.map((item) => `${item.name} [${item.id}]: canonical home ${item.home_location} on ${item.home_side || "the declared"} side; current status ${item.status}; camera-side quantity ${item.camera_quantity}; other-side limit ${item.other_side_quantity_limit}; whole-story limit ${item.global_quantity_limit}. ${item.rule} Adjacent scenes: ${item.adjacent_visibility.map((entry) => `scene ${entry.scene_number} ${entry.location} = ${entry.status}`).join(", ")}`).join(" | ")}. Never invent a second instance or move one into a nearby setting.` : "",
